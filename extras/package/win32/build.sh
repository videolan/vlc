...
if [ -n "$PREBUILT_FAILED" ]; then
    ...
    CFLAGS="$VLC_CFLAGS" CXXFLAGS="$VLC_CXXFLAGS" CPPFLAGS="$VLC_CPPFLAGS" LDFLAGS="$VLC_LDFLAGS" AR="$VLC_AR" RANLIB="$VLC_RANLIB" PKG_CONFIG="$VLC_PKG_CONFIG" PKG_CONFIG_LIBDIR="$VLC_PKG_CONFIG_LIBDIR" make -j$JOBS tools
fi
cd ../..
