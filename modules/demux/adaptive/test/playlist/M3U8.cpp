...
int M3U8Playlist_test()
{
    ...
    try
    {
        ...
        Expect(seg);
        Expect(static_cast<HLSSegment *>(seg)->getUTCTime() == VLC_TS_0 + vlc_tick_from_sec(10));
        seg = rep->getMediaSegment(11);
        Expect(seg);
        Expect(static_cast<HLSSegment *>(seg)->getUTCTime() == VLC_TS_0 + vlc_tick_from_sec(10 + 8));

        /* date change after discontinuity */
        seg = rep->getMediaSegment(20);
        Expect(seg);
        Expect(static_cast<HLSSegment *>(seg)->getUTCTime() == VLC_TS_0 + vlc_tick_from_sec(7200));
