...
#define PREROLL_FORCED VLC_TICK_MAX
    ...
    vlc_tick_t request_delay, output_delay;
...
static void ModuleThread_QueueVideo( decoder_t *p_dec, picture_t *p_pic )
{
    ...
    if ( tracer != NULL )
    {
        vlc_tracer_Trace(tracer, VLC_TRACE("type", "VIDEO"),
...
static void *DecoderThread( void *p_data )
{
    ...
    while( !p_owner->aborting )
    {
        ...
        {   /* Update playing/paused status of the output */
            Decoder_ChangePause( p_owner, p_owner->paused, p_owner->pause_date );
            continue;
        }

        if( p_owner->request_rate != p_owner->output_rate )
        {
            Decoder_ChangeRate( p_owner, p_owner->request_rate );
            continue;
        }

        if( p_owner->request_delay != p_owner->output_delay )
        {
            Decoder_ChangeDelay( p_owner, p_owner->request_delay );
            continue;
        }
...
CreateDecoder( vlc_object_t *p_parent, const struct vlc_input_decoder_cfg *cfg )
{
    ...
    p_owner->output_delay = p_owner->request_delay = 0;
    ...
    p_owner->cc.b_supported = ( p_owner->p_sout == NULL );
